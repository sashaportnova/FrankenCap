...
def augmentTRC(pathInputTRCFile, subject_mass, subject_height,
    ...
    if augmenter_model == 'v0.0':
        ...
        # SP: upper-body marker augmenter only
        augmenterModelType_all = [augmenterModelType_upper]
        feature_markers_all = [feature_markers_upper]
        response_markers_all = [response_markers_upper]
        #
    ...
    for idx_augm, augmenterModelType in enumerate(augmenterModelType_all):
        ...
        # # SP: Add markers to .trc file if they are in the allowed list, and remove existing ones.
        # allowed_markers = ['Nose','Neck','RShoulder','RElbow','RWrist'
        #                    'LShoulder','LElbow','LWrist','REye','LEye',
        #                    'REar','LEar','r_shoulder_study','L_shoulder_study',
        #                    'C7_study','r_lelbow_study','r_melbow_study',
        #                    'r_lwrist_study','r_mwrist_study','L_lelbow_study',
        #                    'L_melbow_study','L_lwrist_study','L_mwrist_study',
        #                    'r.ASIS_study','L.ASIS_study','r.PSIS_study','L.PSIS_study',
        #                    'RHJC_study','LHJC_study']
        # remove_markers = ['midHip','RHip','RKnee','RAnkle',
        #                    'LHip','LKnee','LAnkle','LBigToe','LSmallToe',
        #                    'LHeel','RBigToe','RSmallToe','RHeel']
        # for c, marker in enumerate(response_markers):
        #     if marker in allowed_markers:  # Only add if marker is in the allowed list
        #         x = unnorm2_outputs[:, c * 3]
        #         y = unnorm2_outputs[:, c * 3 + 1]
        #         z = unnorm2_outputs[:, c * 3 + 2]
        #         trc_file.add_marker(marker, x, y, z)
        # for marker in remove_markers:
        #     if marker in trc_file.marker_names:  # Remove existing marker
        #         trc_file.remove_marker(marker)
