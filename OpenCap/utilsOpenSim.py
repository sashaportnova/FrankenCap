...
def runScaleTool(pathGenericSetupFile, pathGenericModel, subjectMass,
    ...
    markerSetFileName = 'MASI_markers.xml'
    ...
    os.makedirs(os.path.dirname(pathUpdGenericModel), exist_ok=True)
    ...
    modelScaler.setOutputModelFileName(pathOutputModel)       
    ...
    print("Marker file set to:", markerPlacer.getMarkerFileName())            
    ...
    # Run scale tool.  
    os.environ['OPENSIM_HOME'] = r'C:\OpenSim 4.5'                    
    ...
    problematic_bodies = ['spine']
    
    for i in range(nBodies):
        bodyName = bodySet.get(i).getName()
        
        #Skip the abdoment body
        if bodyName in problematic_bodies:
            print(f"Skipping {bodyName} to prevent a crash.")
            scale_factors[i, :] = np.nan  # Assign NaNs to indicate missing data
            continue

        body = bodySet.get(bodyName)
        attached_geometry = body.get_attached_geometry(0)  # This might fail
        scale_factors[i, :] = attached_geometry.get_scale_factors().to_numpy()

        # attached_geometry = body.get_attached_geometry(0)
        # scale_factors[i, :] = attached_geometry.get_scale_factors().to_numpy()
    diff_scale = np.max(np.nanmax(scale_factors, axis=0)-
                        np.nanmax(scale_factors, axis=0))
...
def runIKTool(pathGenericSetupFile, pathScaledModel, pathTRCFile,
    ...
    command = f'opensim-cmd -o error run-tool "{pathOutputSetup}"'
    os.system(command)
...
def getScaleTimeRange(pathTRCFile, thresholdPosition=0.005, thresholdTime=0.3,
    ...
    markers = ['Neck','Nose','REye','LEye','RShoulder','RElbow','RWrist',
               'LShoulder','LElbow','LWrist','RHip','LHip']
