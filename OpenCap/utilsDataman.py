...
class TRCFile(object):
    ...
    def remove_marker(self, name):
        """Remove a marker and its data from the TRCFile.
        ...
        if name not in self.marker_names:
            warnings.warn(f"Marker '{name}' not found in the TRC file.")
            return
        ...
        # Remove marker data (x, y, z components)
        fields_to_remove = [f"{name}_tx", f"{name}_ty", f"{name}_tz"]
        self.data = np.lib.recfunctions.drop_fields(self.data, fields_to_remove)
