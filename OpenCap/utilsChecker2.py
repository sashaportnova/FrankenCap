...
def getVideoLength(filename):
        ...
        try:
            # Decode the output and convert to float
            return float(result.stdout.decode().strip())
        except ValueError as e:
            raise ValueError(f"Failed to parse video duration. ffprobe output: {result.stdout.decode().strip()}") from e
...
# %%
def video2Images2(videoPath, nImages=12, tSingleImage=None, filePrefix='output', skipIfRun=True, outputFolder='default'):
    """
    Extracts images from a video file using ffmpeg.
    
    Parameters:
        videoPath (str): Path to the video file.
        nImages (int): Number of images to extract.
        tSingleImage (float): Extract a single image at the specified time (in seconds).
        filePrefix (str): Prefix for output image filenames.
        skipIfRun (bool): Skip extraction if the first image already exists.
        outputFolder (str): Directory to save the output images.
    Returns:
    ...
    first_image_path = os.path.join(outputFolder, f"{filePrefix}0.png")
    if os.path.exists(first_image_path) and skipIfRun:
        print(f"Images already exist. Skipping extraction.")
        return [os.path.join(outputFolder, f"{filePrefix}{i}.png") for i in range(nImages)]

    extracted_images = []

    if tSingleImage is not None:  # Extract a single image
        output_image_path = os.path.join(outputFolder, f"{filePrefix}0.png")
        CMD = [
            'ffmpeg', '-loglevel', 'error', '-y', '-ss', str(tSingleImage),
            '-i', videoPath, '-q:v', '1', '-frames:v', '1', output_image_path
        ]
        subprocess.run(CMD, check=True)
        extracted_images.append(output_image_path)
    else:  # Extract multiple images
        lengthVideo = getVideoLength(videoPath)
        timeImageSamples = np.linspace(1, lengthVideo - 1, nImages)  # Exclude the first and last seconds
        for i, t_image in enumerate(timeImageSamples):
            output_image_path = os.path.join(outputFolder, f"{filePrefix}{i}.png")
            CMD = [
                'ffmpeg', '-loglevel', 'error', '-y', '-ss', str(t_image),
                '-i', videoPath, '-q:v', '1', '-frames:v', '1', output_image_path
            ]
            subprocess.run(CMD, check=True)
            extracted_images.append(output_image_path)
...
# %%                
def calcIntrinsics(folderName, CheckerBoardParams=None, filenames=['*.png'], 
                   imageScaleFactor=1, visualize=False, saveFileName=None):
    
    videoPath = os.path.join(folderName, 'extrinsics.mp4');
    imagePath = os.path.join(folderName, 'extrinsicImage0.png')
    ...
    for iTime,t in enumerate(tSampPts):
        ...
        while not os.path.exists(imagePath) and t>=0:
            video2Images2(videoPath, nImages=1, tSingleImage=t, filePrefix='extrinsicImage', skipIfRun=False)
    ...
    for iImage, pathName in enumerate(imageFiles):
        ...
        if ret == True: 
            ...
            if meta.shape[::-1] != CheckerBoardParams['dimensions']:
                print(f"Mismatch in dimensions: expected {CheckerBoardParams['dimensions']}, got {meta.shape[::-1]}")
                return None
...
# %% My own code
def calc_extrinsics(image_file, camera_params, checkerboard_params, visualize=False):
    """
    Calculate camera extrinsics using a calibration image with a checkerboard.

    Args:
        image_file (str): Path to the input image.
        camera_params (dict): Dictionary with 'intrinsicMat' and 'distortion' arrays.
        checkerboard_params (dict): Dictionary with 'dimensions' and 'square_size' (e.g., in mm).
        visualize (bool): Whether to visualize the detected checkerboard and axes.

    Returns:
        dict: Extrinsic parameters including rotation matrix and translation vector.
    """
    ...
    # Generate the 3D points for the checkerboard in the world coordinate system
    rows, cols = checkerboard_params['dimensions']
    square_size = checkerboard_params['squareSize']
    ...
    ret, corners = cv2.findChessboardCorners(gray, (cols, rows), cv2.CALIB_CB_ADAPTIVE_THRESH)
    if not ret:
    ...
    ret, rvec, tvec = cv2.solvePnP(
        ...
        camera_params['intrinsicMat'], camera_params['distortion']
    )
    if not ret:
    ...
    R_world_from_camera, _ = cv2.Rodrigues(rvec)

    # Prepare output
    extrinsics = {
        'rotation': R_world_from_camera,
        'translation': tvec,
        'rotation_EulerAngles': rvec,
        'distortion': camera_params['distortion'],
        'imageSize': camera_params['imageSize'],
        'intrinsicMat': camera_params['intrinsicMat']
    }
    ...
    saveExtPath = os.path.join(
        ...
        'cameraIntrinsicsExtrinsics_soln{}.pickle')
    saveCameraParameters(saveExtPath,extrinsics)

    # Visualize checkerboard and axes (if requested)
    if visualize:
        img_points, _ = cv2.projectPoints(
            np.array([[0, 0, 0], [square_size, 0, 0], [0, square_size, 0]]),
            rvec, tvec, camera_params['intrinsicMat'], camera_params['distortion']
        )
        ...
        # Draw checkerboard and axes
        vis_image = cv2.drawChessboardCorners(image, (cols, rows), corners_refined, ret)
        cv2.line(vis_image, origin, x_axis, (0, 0, 255), 3)  # X-axis in red
        cv2.line(vis_image, origin, y_axis, (0, 255, 0), 3)  # Y-axis in green

        # Display the image
        cv2.imshow("Checkerboard and Axes", vis_image)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
...
def calcExtrinsicsFromVideo(videoPath, CamParams, CheckerBoardParams,
    ...
    for iTime,t in enumerate(tSampPts):
        ...
        if not os.path.exists(imagePath):
            video2Images2(videoPath, nImages=1, tSingleImage=0.01, filePrefix='extrinsicImage', skipIfRun=False)
        ...
        # CamParamsTemp = calc_extrinsics(
        #     os.path.join(videoDir, 'extrinsicImage0.png'),
        #     CamParams, CheckerBoardParams, visualize=False)
...
def synchronizeVideos(CameraDirectories, trialRelativePath, pathPoseDetector,
    ...
    # upperBodyMarkers = ["Nose", "Neck", "RShoulder", "RElbow", "RWrist",
    #                     "LShoulder", "LElbow", "LWrist", "REye",
    #                     "LEye", "REar", "LEar"]
    ...
    for camName in CameraDirectories_selectedCams:
        ...
        thisVideo = cv2.VideoCapture(videoFullPath.replace('.mp4', '_rotated.avi'))
    ...
    pointList,confList, nansInOutList, startEndFrameList = synchronizeVideoKeypoints2(pointList, confList,
                                  confidenceThreshold=0.3, 
                                  filtFreq=12, sampleFreq=frameRate, 
                                  maxShiftSteps=2*frameRate, gaitType="eyes")
...
# %%
def synchronizeVideoKeypoints2(keypointList, confidenceList,
                              confidenceThreshold=0.3, 
                              filtFreq=12, sampleFreq=30, 
                              maxShiftSteps=30, gaitType="walking"):
    print('Synchronizing Keypoints')
    
    # Deep copy inputs to avoid modification
    keypointList = copy.deepcopy(keypointList)
    confidenceList = copy.deepcopy(confidenceList)
    
    # Define ankle marker indices
    markerNames = getOpenPoseMarkerNames()
    mkrDict = {mkr: iMkr for iMkr, mkr in enumerate(markerNames)}
       
    if gaitType == "walking":
        markers4Ankles = [mkrDict['RAnkle'], mkrDict['LAnkle']]
    elif gaitType == "wheelchair":
        markers4Ankles = [mkrDict['RWrist'], mkrDict['LWrist']]
    elif gaitType == "eyes":
        markers4Ankles = [mkrDict['REar'], mkrDict['LEar']]
    
    # Remove occluded markers
    # keypointList, confidenceList = zip(*[removeOccludedSide(keys, conf, {'right': [mkrDict['RAnkle']], 'left': [mkrDict['LAnkle']]}, confidenceThreshold, visualize=False) for keys, conf in zip(keypointList, confidenceList)])
    
    # Compute marker speeds
    # nCams = len(keypointList)
    # mkrSpeedList = []
    # keyFiltList = []
    # confFiltList = []
    ...
    # nansInOutList = []
    # for keyRaw, conf in zip(keypointList, confidenceList):
    #     keyRaw_clean, conf_clean, nans_in_out, conf_sync_clean = clean2Dkeypoints(keyRaw, conf, confidenceThreshold, nCams, linearInterp=True)
    #     keyRaw_clean_filt = filterKeypointsButterworth(keyRaw_clean, filtFreq, sampleFreq, order=4)
    #     keyFiltList.append(keyRaw_clean_filt)
    #     confFiltList.append(conf_clean)
    #     confSyncFiltList.append(conf_sync_clean)
    #     nansInOutList.append(nans_in_out)
    #     mkrSpeedList.append(getMarkerSpeed(keyRaw_clean_filt, markers4Ankles, confidence=conf, averageVels=False))
    
    # # Find overlapping confidence regions
    # overlapInds_clean, minConfLength_all = findOverlap(confidenceList, markers4Ankles)
    # if not np.any(overlapInds_clean):
    #     return [np.zeros((keypointList[0].shape[0], 10, keypointList[0].shape[2])) for _ in range(nCams)], \
    #            [np.zeros((keypointList[0].shape[0], 10)) for _ in range(nCams)], \
    #            [np.array([np.nan, np.nan]) for _ in range(nCams)], \
    #            [None] * nCams
    
    # idxStart, idxEnd = int(np.min(overlapInds_clean)), int(np.max(overlapInds_clean)) + 1
    # idxStart = max(0, idxStart - maxShiftSteps)
    # idxEnd = min(idxEnd + maxShiftSteps, minConfLength_all)
    # idxStart = int(idxStart)
    # idxEnd = int(idxEnd)
    # mkrSpeedList = [v[:, idxStart:idxEnd] for v in mkrSpeedList]
    
    # # Synchronize cameras using ankle marker speeds
    # shiftVals = [0]
    # timeVecs = [np.arange(keypointList[0].shape[1])]
    # for iCam in range(1, nCams):
    #     if np.max(np.abs(mkrSpeedList[iCam])) == 0 or np.max(np.abs(mkrSpeedList[0])) == 0:
    #         lag = 0
    #     else:
    #         _, lag = cross_corr_multiple_timeseries(mkrSpeedList[iCam], mkrSpeedList[0], maxShiftSteps / 2, frameRate=sampleFreq)
    #     lag = 0 if abs(lag) > maxShiftSteps else lag
    #     shiftVals.append(lag)
    #     timeVecs.append(np.arange(keypointList[iCam].shape[1]) - lag)
    # # 
    # tStart, tEnd = max(t[0] for t in timeVecs), min(t[-1] for t in timeVecs)
    
    # # Keeping only the upper-body markers
    # # upperBodyMarkers = ["Nose", "Neck", "RShoulder", "RElbow", "RWrist",
    # #                     "LShoulder", "LElbow", "LWrist", "REye",
    # #                     "LEye", "REar", "LEar"]
    # # upperBodyIndices = [mkrDict[mkr] for mkr in upperBodyMarkers]
    
    # keypointsSync, confidenceSync, nansInOutSync, startEndFrames = [], [], [], []
    # for iCam, key in enumerate(keypointList):
    #     confidence = confidenceList[iCam]
    #     iStart, iEnd = int(np.argwhere(timeVecs[iCam] == tStart)[0][0]), int(np.argwhere(timeVecs[iCam] == tEnd)[0][0])
    #     keypointsSync.append(key[:, iStart:iEnd+1, :])
    #     confidenceSync.append(confidence[:, iStart:iEnd+1])
    #     nansInOutSync.append(np.array([np.nan, np.nan]) - shiftVals[iCam])
    #     startEndFrames.append([iStart, iEnd])
        
        
        
    # Find the number of frames in the keypoints list with the smallest frame count
    min_frames = min([kp.shape[1] for kp in keypointList])
    
    # Trim keypoints and confidences to the smallest number of frames
    trimmed_keypoints = [kp[:,:min_frames,:] for kp in keypointList]
    trimmed_confidences = [conf[:,:min_frames] for conf in confidenceList]
    
    startEndFrames = []
    nansInOutSync = []
    for iCam in enumerate(keypointList):
        iStart = 0
        iEnd = min_frames
        startEndFrames.append([iStart, iEnd])
        nansInOutSync.append([np.nan, np.nan])
        
    # return keypointsSync, confidenceSync, nansInOutSync, startEndFrames
    return trimmed_keypoints, trimmed_confidences, nansInOutSync, startEndFrames
# %%
...
def trackKeypointBox(videoPath,bbStart,allPeople,allBoxes,dataOut,frameStart = 0 ,
    ...
    # Read video
    video = cv2.VideoCapture(videoPath.replace('.mp4', '_rotated.avi'))
...
def triangulateMultiviewVideo(CameraParamDict,keypointDict,imageScaleFactor=1,
    ...
    if trimTrial:
        ...
        # nZeroConf = keypointList_selectedCams[0].shape[0] - np.count_nonzero(confidence3D, axis=1)
        markerNames = getOpenPoseMarkerNames()        
        allMkrInds = np.arange(len(markerNames))
        _, idxFaceMarkers = getOpenPoseFaceMarkers()        
        # SP: Let's only feed in upper-body markers:
        markersToRemove = [ "midHip", "RHip","RKnee", "RAnkle", "LHip", "LKnee", 
                           "LAnkle","LBigToe", "LSmallToe",
                           "LHeel", "RBigToe", "RSmallToe", "RHeel",
                           'Nose', 'REye', 'LEye', 'REar', 'LEar'] 
        indicesToRemove = [i for i, marker in enumerate(markerNames) if marker in markersToRemove]
        # SP
        includedMkrs = np.delete(allMkrInds,indicesToRemove)
        nZeroConf = (len(includedMkrs) - 
                     np.count_nonzero(confidence3D[:,includedMkrs,:],axis=1))        
        if not True in (nZeroConf<1).flatten():
            # points3D = np.zeros((3, keypointList_selectedCams[0].shape[0], 10))
            # confidence3D = np.zeros((1, keypointList_selectedCams[0].shape[0], 10))
    ...
    if CameraDirectories != None and trialName !=None:       
        ...
        for iCam,camName in enumerate(keypointDict):
            ...
            if iCam==0: 
                thisVideo = cv2.VideoCapture(inputPath.replace('.mp4', '_rotated.avi'))
                frameRate = np.round(thisVideo.get(cv2.CAP_PROP_FPS))
                thisVideo.release()
    ...
    # visualize the results of triangulation in the browser
    visualization = False
    if visualization:
        # Flatten the points3D array into a 1D array for plotting
        x = points3D[0, :].flatten()
        y = points3D[1, :].flatten()
        z = points3D[2, :].flatten()
    ...
    # Zero out the lower-body marker coordinates
    markersToRemove = ['midHip','RHip','RKnee','RAnkle',
                       'LHip','LKnee','LAnkle','LBigToe','LSmallToe',
                       'LHeel','RBigToe','RSmallToe','RHeel']
    for marker in markersToRemove:
        if marker in markerNames:
            markerRemove = markerNames.index(marker)
            points3D[:, markerRemove, :] = 0
            confidence3D[:, markerRemove, :] = 0
...
def writeTRCfrom3DKeypoints(keypoints3D, pathOutputFile, keypointNames, 
    ...
    # SP: remove the lower-body markers from the TRC file
    # markersToRemove = ["midHip", "RHip",
    # "RKnee", "RAnkle", "LHip", "LKnee", "LAnkle",
    # "LBigToe", "LSmallToe",
    # "LHeel", "RBigToe", "RSmallToe", "RHeel"]
    # idxMarkersToRemove = [keypointNames.index(i) for i in markersToRemove]  
    # idxToRemove = np.hstack([np.arange(i*3,i*3+3) for i in idxMarkersToRemove])
    # keypoints3D_res_sel = np.delete(keypoints3D_res, idxToRemove, axis=1)  
    # keypointNames_sel = [i for i in keypointNames if i not in markersToRemove]
    #
...
def loadPklVideo(pklPath, videoFullPath, imageBasedTracker=False, poseDetector='OpenPose',
    ...
    if len(allPeople) >1: 
        ...
        #Plot the first frame of all the people found
        # videoPath = videoFullPath
        # image_path = video2Images(videoPath, nImages=1, tSingleImage=0, filePrefix='output', skipIfRun=True, outputFolder='default')
        ...
        #     for j in range(0,25):
        #         key2D[j,:,0:2] = allPeople[i][:,j*3:j*3+2]          
        #     ax.scatter(key2D[:,0,0], key2D[:,0,1], label=f'Person {i}')
        # plt.legend()
        # plt.show()
        
        # user_input = input("Select the person you want to track throughout the alogirthm: ")
        # personToTrack = int(user_input)
        ...
        def plot_frame(frame, allPeople,startFrame):
            fig, ax = plt.subplots()
        
            # Convert NumPy frame (BGR) to PIL image (RGB) if necessary
            ...
            for i in range(len(allPeople)):
                if allPeople[i].size > 0:
                    key2D = np.zeros((25,nFrames,2))  # Keypoints for 25 joints (per person)
                    for j in range(25):
                        key2D[j, :, 0:2] = allPeople[i][:, j * 3:j * 3 + 2]
                    ax.scatter(key2D[:,startFrame,0], key2D[:,startFrame,1], label=f'Person {i}')
                    has_people = True
        
            if has_people:
                ax.legend()
            plt.show()  # Show the plot in a blocking way
        ...
        def frame_navigation(videoFrames, allPeople):
            startFrame = 0
            personToTrack = None
        
            while startFrame < len(videoFrames):
                frame = videoFrames[startFrame]
        
                # Ensure the frame is valid
                if frame is not None and frame.size > 0:
                    plot_frame(frame, allPeople,startFrame)
                else:
                ...
                user_input = input("Is the person you want to track visible in the frame? (y/n): ").lower()
        
                if user_input == 'y':
                    # Select the person
                    personToTrack = int(input("Select the person you want to track: "))
                    ...
                    for i in range(len(allPeople)):
                        if i == personToTrack:
                            for j in range(25):
                                key2D[j, :, 0:2] = allPeople[i][:, j * 3:j * 3 + 2]
                                confidence[j, :] = allPeople[i][:, j * 3 + 2]
                            break
        
                    # End the loop and return data
                    plt.show()
                    ...
                    print("Invalid input. Press 'y' to select the person or 'n' to skip to the next frame.")
        ...
        # Main logic
        videoPath = videoFullPath
        videoFrames = load_video_frames(videoPath)
        
        personToTrack, startFrame = frame_navigation(videoFrames, allPeople)
